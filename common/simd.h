...
#include "int.h"
    ...
    #define simd_crc32c_u8(crc, val) (assert(false && "CRC32C SIMD not available on Android!"), 0)
    #define simd_crc32c_u16(crc, val) (assert(false && "CRC32C SIMD not available on Android!"), 0)
    #define simd_crc32c_u32(crc, val) (assert(false && "CRC32C SIMD not available on Android!"), 0)
    #define simd_crc32c_u64(crc, val) (assert(false && "CRC32C SIMD not available on Android!"), 0)
#else
    #ifdef _MSC_VER
        /// MSVC CPUID
        #include <immintrin.h>
